%Optimization On Stiefel Manifolds written in terms of MATLAB class
...
classdef Class_GD_Stiefel_Euclid
...
function self = Class_GD_Stiefel_Euclid(omega, Seq, iteration, lr, lrdecayrate, gradnormthreshold, checkonStiefelthreshold)           
    if nargin > 0  
        self.omega = omega;  
        self.Seq = Seq;  
...
%Given objective function f_F(A)=\sum_{k=1}^m \omega_k \|A-A_k\|_F^2 where A, A_k\in St(p, n)
%Use Gradient Descent to find min_A f_F(A)
function [fseq, gradfnormseq, distanceseq, minf] = GD_Stiefel(self, Y)
    ...
    for i = 1:self.iteration
        ...
        [f, gradf] = self.gradientStiefel(A);
...
function [f, gradf] = gradientStiefel(self, Y)
    m = length(self.omega);
    f = 0;
    for i = 1:m
